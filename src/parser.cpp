...
std::shared_ptr<Statement::Stmt> Parser::statement(){
  ...
  if(check(TokenType::IDENTIFIER)){
    ...
    if(next < tokens.size()){
      ...
      if(nt == TokenType::LEFT_PAREN){
        Token unknown = peek();
        throw error(unknown, "'" + unknown.lexeme + "' is not a recognized command in .su — did you mean 'proclaim()'?");
      }
    }
  }
  ...
  if(increment != nullptr){
    body = std::make_shared<Statement::Block>(
      std::vector<std::shared_ptr<Statement::Stmt>>{
        body,
        std::make_shared<Statement::Expression>(increment)
      }
    );
  }
